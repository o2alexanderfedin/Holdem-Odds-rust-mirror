...
use crate::hands_h::{
    Flush, FourOfAKind, FullHouse, HandType, HighCard, Pair, Straight,
    StraightFlush, ThreeOfAKind, TwoPair, Wheel, WheelFlush,
};
...
extern "C" fn hand_is_straight(cards: *const Card) -> bool {
    return unsafe { (*cards.offset(0 as isize)).rank } ==
                unsafe {
                    (*cards.offset(1 as isize)).rank.wrapping_add(1 as u32)
                } &&
            unsafe { (*cards.offset(0 as isize)).rank } ==
                unsafe {
                    (*cards.offset(4 as isize)).rank.wrapping_add(4 as u32)
                };
}
...
pub(crate) extern "C" fn hand_classify(cards: *mut Card) -> HandType {
    if hand_is_flush(cards as *const Card) {
        if hand_is_straight(cards as *const Card) { return StraightFlush; }
        ...
        if hand_is_full_house(cards as *const Card) { return FullHouse; }
        if hand_is_straight(cards as *const Card) { return Straight; }
        if unsafe { hand_is_wheel(cards as *const Card) } { return Wheel; }
        if hand_is_three_of_a_kind(cards as *const Card) {
            return ThreeOfAKind;
        }
        if hand_is_two_pair(cards as *const Card) { return TwoPair; }
        if hand_is_pair(cards as *const Card) { return Pair; }
        return HighCard;
    }
}

pub(crate) extern "C" fn hand_compare(hand1: *mut Card, hand2: *mut Card)
    -> i32 {
    ...
    if diff != 0 { return diff; }
    {
        ...
        '__b3: loop {
            if !(i < 5 as u64) { break '__b3; }
            '__c3: loop {
                let cmp: i32 =
                    card_compare(unsafe { &*hand1.add(i as usize) },
                        unsafe { &*hand2.add(i as usize) });
                if cmp != 0 { return cmp; }
                break '__c3;
            }
            { let __p = &mut i; *__p = (*__p).wrapping_add(1); *__p };
...
pub(crate) extern "C" fn hand_sort(hand: *mut Card) -> () {
    let mut buckets: [Bucket; 13] = unsafe { core::mem::zeroed() };
    {
        let mut i: u64 = 0 as u64;
        '__b4: loop {
            if !(i < 5 as u64) { break '__b4; }
            '__c4: loop {
                let bucket_index: u64 =
                    unsafe {
                            (*hand.add(i as usize)).rank.wrapping_sub(1 as u32)
                        } as u64;
                bucket_add(&mut buckets[bucket_index as usize],
                    unsafe { &*hand.add(i as usize) });
                break '__c4;
            }
            { let __p = &mut i; *__p = (*__p).wrapping_add(1); *__p };
        }
    ...
    let mut index: u64 = 0 as u64;
    {
        ...
        '__b5: loop {
            if !(count > 0 as u64) { break '__b5; }
            '__c5: loop {
                {
                    let mut j: u64 = (13 - 1) as u64;
                    '__b6: loop {
                        if !(j != -1i32 as u64) { break '__b6; }
                        '__c6: loop {
                            if buckets[j as usize].count == count {
                                unsafe {
                                    __builtin___memcpy_chk(unsafe {
                                                &raw mut *hand.add(index as usize)
                                            } as *mut (),
                        ...
                        { let __p = &mut j; *__p = (*__p).wrapping_sub(1); *__p };
                    }
                }
                break '__c5;
            }
            { let __p = &mut count; *__p = (*__p).wrapping_sub(1); *__p };
        }
    }
}
...
pub(crate) extern "C" fn hand_is_valid(cards: *const Card) -> bool {
    {
        let mut i: u64 = 0 as u64;
        '__b7: loop {
            if !(i < 5 as u64) { break '__b7; }
            '__c7: loop {
                if !unsafe {
                                    card_is_valid(unsafe { &*cards.add(i as usize) })
                                } as i32 != 0 {
                    return 0;
                }
                break '__c7;
