use super::*;
use crate::cards_h::{Card, Rank, Suit};
...
pub(crate) extern "C" fn new_card(rank: Rank, suit: Suit) -> Card {
    unsafe {
        let mut card: Card = Card { rank: rank, suit: suit };
...
pub(crate) extern "C" fn char_to_rank(r: i8) -> Rank {
    return index_of(r, c"23456789TJQKA".as_ptr() as *mut i8 as *const i8,
                13 as u64) as Rank;
}

pub(crate) extern "C" fn char_to_suit(s: i8) -> Suit {
    return index_of(s, c"cdhs".as_ptr() as *mut i8 as *const i8, 4 as u64) as
            Suit;
}
...
pub(crate) extern "C" fn new_card_from_string(s: *mut i8) -> Card {
    unsafe {
        if unsafe { strlen(s as *const i8) } != 2 as u64 {
            ...
            return new_card_from_chars(unsafe { *s.offset(0 as isize) },
                    unsafe { *s.offset(1 as isize) });
        }
    }
}
    ...
    if !unsafe { card_is_valid(c) } as i32 != 0 {
        ...
        unsafe {
            *out.offset(0 as isize) =
                unsafe {
                    *(c"23456789TJQKA".as_ptr() as
                                *mut i8).offset((unsafe { (*c).rank } as i32 - 1) as isize)
                }
        };
        unsafe {
            *out.offset(1 as isize) =
                unsafe {
                    *(c"cdhs".as_ptr() as
                                *mut i8).offset((unsafe { (*c).suit } as i32 - 1) as isize)
                }
        };
    }
