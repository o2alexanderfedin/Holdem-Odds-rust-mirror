...
pub(crate) extern "C" fn __main_inner(argc: i32, argv: *const *mut i8)
    -> Result<(), i32> {
    if argc != 5 { usage(unsafe { *argv.offset(0 as isize) } as *const i8); }
    ...
    let mut results: [u64; 3] = [0 as u64, 0 as u64, 0 as u64];
    {
        ...
        '__b8: loop {
            if !(i < 4 as u64) { break '__b8; }
            '__c8: loop {
                cards[i as usize] =
                    new_card_from_string(unsafe {
                            *argv.add(i.wrapping_add(1 as u64) as usize)
                        });
                break '__c8;
            }
    ...
    unsafe {
        new_deck(&raw mut deck[0 as usize] as *mut Card,
            &raw mut cards[0 as usize] as *mut Card as *const Card, 4 as u64)
    };
    {
        let mut i: u64 = 0 as u64;
        '__b9: loop {
            if !(i < 2000000 as u64) { break '__b9; }
            '__c9: loop {
                deck_shuffle(&raw mut deck[0 as usize] as *mut Card, 3 as u64,
                    48 as u64);
                ...
                hand2[1 as usize] = cards[3 as usize];
                {
                    ...
                    '__b10: loop {
                        if !(j < 3 as u64) { break '__b10; }
                        '__c10: loop {
                            hand1[(2 as u64).wrapping_add(j) as usize] =
                                deck[j as usize];
                            hand2[(2 as u64).wrapping_add(j) as usize] =
                                deck[j as usize];
                            break '__c10;
                        }
                ...
                let c: i32 =
                    hand_compare(&raw mut hand1[0 as usize] as *mut Card,
    ...
    unsafe {
        printf(c"WIN: %.2f\tTIE: %.2f\tLOSS: %.2f\n".as_ptr() as *mut i8 as
                *const i8, results[0 as usize] as f64 / 2000000 as f64,
            results[1 as usize] as f64 / 2000000 as f64,
            results[2 as usize] as f64 / 2000000 as f64)
    };
    return Ok(());
}
